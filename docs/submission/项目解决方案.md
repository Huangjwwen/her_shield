...
flowchart TB
    ...
    subgraph "后端代理层 — Tencent CloudBase"
        Proxy["云函数 proxy<br/>密钥下沉 + 缓存 + CORS"]
        DB[("云数据库<br/>她声故事")]
    end

    subgraph "智能层 — 腾讯元器"
        Y1["她眼·言行雷达<br/>(三节点工作流)"]
        Y2["她权·权益指南<br/>(单节点)"]
        Y3["她证·证据保全<br/>(单节点)"]
        Y4["她行·维权导航<br/>(单节点)"]
        Y5["她心·情绪树洞<br/>(prompt 直驱)"]
    end

    subgraph "知识层"
        Deli["得理开放平台<br/>类案 + 法规检索"]
        BeiDa["北大法宝<br/>法律法规库"]
        Static["静态法条注入<br/>System Prompt"]
    end
    ...
    Modules -->|HTTPS POST<br/>fetch with Origin| Proxy
    Proxy -->|appkey Bearer<br/>转发| Y1
    Proxy -->|appkey Bearer| Y2
    Proxy -->|appkey Bearer| Y3
    Proxy -->|appkey Bearer| Y4
    Proxy -->|appkey Bearer| Y5
...
| **后端代理层** | 腾讯 CloudBase 云函数 + 云数据库 + HTTP 触发器 | API 代理转发;**5 个 appkey 下沉到环境变量**;同问缓存治理"同问不同答";CORS 头处理 |
| **智能层** | 腾讯元器 5 大智能体(可视化工作流编排) | 意图识别、要件抽取、要件式评估、法条引用、类案匹配、温暖叙事、三级话术 |
| **知识层** | 得理开放平台 API + 北大法宝 + 静态法条注入 | 法规检索(RAG)、类案检索(RAG)、System Prompt 内置法条 |
